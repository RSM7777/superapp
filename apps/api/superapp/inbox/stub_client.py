...
class StubMailClient:
    ...
    def history(self, *, months: int = 24, limit: int = 1500) -> list[dict]:
        """The offline mailbox has no archive and no sent folder, so there is
        no past to import. Empty, not fabricated: a test that reads invented
        history as real history proves nothing."""
        return []
